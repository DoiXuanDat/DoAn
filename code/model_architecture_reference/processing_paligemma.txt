...
def process_images(images: List[Image.Image], size: Dict[str, int] = None, resample: Image.Resampling = None,
        scale_factor: float = None, image_mean: Optional[Union[float, List[float]]] = None, image_std: Optional[Union[float, List[float]]] = None
    ) -> List[np.ndarray]:
...
class PaliGemmaProcessor:
    ...
    def __call__(self, text: List[str], images: List[Image.Image], padding: str = "longest", truncation: bool = True) -> dict:
        ...
        pixel_values = process_images( images, size=(self.image_size), resample=Image.Resampling.BICUBIC,
